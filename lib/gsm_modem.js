...
function Modem(opts) {
  ...
  this.resetVars();
// add option for manage device
  this.msidn = opts.msidn || null;
  this.group = opts.group || [];
  this.token = opts.token || 0;
}
  ...
  serialPort.on('error', function (err) {
    if(!this.forever) {
      this.logger.error('Port connect error: %s', err.message);
	  	this.emit('retry', err.message);
		console.log("Error connect port : retry attempt");
    }
...
Modem.prototype.sendNext = function () {
  ...
  if (cmd && !cmd.sent) {
    cmd.startTimer(this.commandsTimeout, function () {
      this.logger.error('command %s timeout', cmd.cmd);
      // this.emit('error', new Error('TIMEOUT'));
      // this.logger.error('command %s timedout', cmd.cmd);
